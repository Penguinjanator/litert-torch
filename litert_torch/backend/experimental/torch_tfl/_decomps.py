...
def _promote_types_for_binary_op(x, y):
  ...
  # TFLite's binary ops require operands to have the same element type, so we
  # promote both operands before calling the op.
  #
  # `torch.result_type` implements PyTorch's promotion rules for mixed
  # tensor/scalar operands, where a Python scalar participates *weakly*: it can
  # widen the result category (int tensor * float scalar -> float) but does not
  # impose its own default width on a tensor operand (fp16 tensor * float
  # scalar stays fp16). Coercing the scalar to the tensor's dtype instead would
  # silently truncate, e.g. int_tensor * 0.5 would become int_tensor * 0.
  if isinstance(x, torch.Tensor) or isinstance(y, torch.Tensor):
    target_dtype = torch.result_type(x, y)
  else:
    # `torch.result_type` requires at least one tensor operand.
    target_dtype = torch.promote_types(
        torch.tensor(x).dtype, torch.tensor(y).dtype
    )

  if not isinstance(x, torch.Tensor):
    x = torch.scalar_tensor(x, dtype=target_dtype)
  elif x.dtype != target_dtype:
    x = x.to(target_dtype)

  if not isinstance(y, torch.Tensor):
    y = torch.scalar_tensor(y, dtype=target_dtype)
  elif y.dtype != target_dtype:
    y = y.to(target_dtype)
